...
import { readHeaders }                                          from './csv-parser.js';
import { initDropzones }                                        from './dropzone.js';
import { buildMappingUI, getFieldMappings, getFileColumn, getFilePathColumn } from './column-mapper.js';
import { submitUpload }                                         from './uploader.js';
import { startPolling }                                         from './poller.js';
import { showToast, setRunButton, showProgressPanel }           from './ui.js';
...
async function startUpload() {
  ...
  const fieldMappings = getFieldMappings();
  const fileCol       = getFileColumn();       // e.g. "col3_file_attachment" → "b.pdf"
  const filePathCol   = getFilePathColumn();   // e.g. "col1_logical_key"     → "c"
  ...
  try {
    jobMeta = await submitUpload({
      ...
      fieldMappings,
      fileCol,
      filePathCol,   // combined with fileCol gives lookup key "c/b.pdf"
      csvFile,
      attachFiles,
    });
  } catch (err) {
