...
export function initDropzones({ onCsv, onFiles }) {
  ...
  // Attachments drop zone — folder mode
  _bindZone('drop-files', 'files-name', {
    folder: true,
    onPick: files => {
      attachFiles = files;
    ...
    label: files => `${files.length} file(s) from folder`,
  });
 ...
 * @param {string}   nameId       - ID of the label element showing selection.
 * @param {Object}   opts
 * @param {boolean}  [opts.folder] - Enable folder picking (webkitdirectory).
 * @param {Function} opts.onPick  - Called with File (single) or File[] (folder).
 * @param {Function} opts.label   - Returns display string given what onPick receives.
 */
function _bindZone(zoneId, nameId, { folder = false, onPick, label }) {
  const zone   = document.getElementById(zoneId);
  const input  = zone.querySelector('input[type=file]');
  const nameEl = document.getElementById(nameId);
